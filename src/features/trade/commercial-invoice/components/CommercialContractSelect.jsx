...
export default function CommercialContractSelect({ contracts, loading, value, onSelect, disabled }) {
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: '#92400E' }}>
              A finalized Contract is required before you can create a Commercial Invoice. Please finalize a contract
              first. Commercial Invoices cannot be created or generated without a valid linked contract.
            </p>
          </div>
        </div>
        <button
    ...
    <div className="p-4 rounded-2xl mb-5" style={{ background: LIGHT, border: `1px solid ${BORDER}` }}>
      ...
      {!value && !fetching && !error && (
        <p className="text-xs mt-1.5" style={{ color: MUTED }}>
          Selecting a contract auto-populates Buyer, Seller, Commodity, Destination and Terms below.
        </p>
      )}
    </div>
  );
