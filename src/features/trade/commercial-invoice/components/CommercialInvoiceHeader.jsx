...
export default function CommercialInvoiceHeader({ register, errors, commercialInvoiceNumber, contractNumber, org }) {
  return (
    <>
      ...
      <Section title="Invoice Information" icon={FileText}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <Field label="Commercial Invoice Number">
            <input value={commercialInvoiceNumber || 'Auto-generated'} disabled className={`${inputCls(false)} bg-gray-50 text-gray-500`} />
          </Field>
          <Field label="Contract Number">
            <input value={contractNumber || '—'} disabled className={`${inputCls(false)} bg-gray-50 text-gray-500`} />
          </Field>
          <Field label="Date" required error={errors.invoiceInfo?.date?.message}>
            <input type="date" {...register('invoiceInfo.date')} className={inputCls(!!errors.invoiceInfo?.date)} />
          </Field>
        </div>
      </Section>
    </>
