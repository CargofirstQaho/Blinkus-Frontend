...
import ProformaInvoiceReview from '../components/ProformaInvoiceReview';
import { BRAND, GRAD, LIGHT, MUTED } from '../components/FormUI';

export default function ProformaInvoiceReviewPage() {
  const [params]  = useSearchParams();
  const navigate  = useNavigate();
  const id        = params.get('id');
  const org = useSelector((s) => s.tradeOrganization.organization);
  const { draft, loading, generating, error, loadById, generatePdf } = useProformaInvoice();
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-64 gap-3">
        ...
        <p className="text-sm" style={{ color: MUTED }}>Loading proforma invoice…</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-xl mx-auto mt-16 px-4">
        <div className="flex items-start gap-3 p-4 rounded-2xl" style={{ background: '#fef2f2', border: '1px solid #fecaca' }}>
          ...
          <div>
            <p className="font-semibold" style={{ color: '#B91C1C' }}>{error || 'Proforma invoice not found'}</p>
            <button onClick={() => navigate('/trade/international/proforma-invoice')} className="text-sm mt-2 underline" style={{ color: '#DC2626' }}>
              Back to Proforma Invoice form
            </button>
          </div>
        </div>
      </div>
  ...
  const pi = draft;
  const isGenerated = pi.status === 'GENERATED' || confirmed;

  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <button onClick={() => navigate('/trade/international/proforma-invoice')} className="flex items-center gap-1.5 text-sm mb-2 hover:opacity-70 transition-opacity" style={{ color: MUTED }}>
            <ArrowLeft size={14} /> Back to form
          </button>
          <h1 className="text-2xl font-extrabold" style={{ color: BRAND }}>Proforma Invoice Review</h1>
          <p className="text-sm mt-0.5 font-mono" style={{ color: MUTED }}>{pi.proformaInvoiceNumber}</p>
        </div>
      ...
      <ProformaInvoiceReview proformaInvoice={pi} org={org} />
