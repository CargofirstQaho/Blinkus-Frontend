...
import DebitNoteReview from '../components/DebitNoteReview';
import { BRAND, GRAD, LIGHT, MUTED } from '../components/FormUI';

export default function DebitNoteReviewPage() {
  const [params]  = useSearchParams();
  const navigate  = useNavigate();
  const id        = params.get('id');
  const org = useSelector((s) => s.tradeOrganization.organization);
  const { draft, loading, generating, error, loadById, generatePdf } = useDebitNote();
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-64 gap-3">
        ...
        <p className="text-sm" style={{ color: MUTED }}>Loading debit note…</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-xl mx-auto mt-16 px-4">
        <div className="flex items-start gap-3 p-4 rounded-2xl" style={{ background: '#fef2f2', border: '1px solid #fecaca' }}>
          ...
          <div>
            <p className="font-semibold" style={{ color: '#B91C1C' }}>{error || 'Debit note not found'}</p>
            <button onClick={() => navigate('/trade/domestic/debit-note')} className="text-sm mt-2 underline" style={{ color: '#DC2626' }}>
              Back to Debit Note form
            </button>
          </div>
        </div>
      </div>
  ...
  const dn = draft;
  const isGenerated = dn.status === 'GENERATED' || confirmed;

  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <button onClick={() => navigate('/trade/domestic/debit-note')} className="flex items-center gap-1.5 text-sm mb-2 hover:opacity-70 transition-opacity" style={{ color: MUTED }}>
            <ArrowLeft size={14} /> Back to form
          </button>
          <h1 className="text-2xl font-extrabold" style={{ color: BRAND }}>Debit Note Review</h1>
          <p className="text-sm mt-0.5 font-mono" style={{ color: MUTED }}>{dn.debitNoteNumber}</p>
        </div>
      ...
      <DebitNoteReview debitNote={dn} org={org} />
